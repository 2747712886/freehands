...
public final class VirtualMainHandContext {
    private static final ThreadLocal<Deque<Entry>> ACTIVE_MINING_STACKS = ThreadLocal.withInitial(ArrayDeque::new);
    ...
    public static void beginMining(Player player, BlockState state) {
        FreeHandEvents.selectedMiningStack(player, state)
                .ifPresent(stack -> ACTIVE_MINING_STACKS.get().push(new Entry(player.getUUID(), stack)));
    }

    public static void endMining(Player player) {
        Deque<Entry> stacks = ACTIVE_MINING_STACKS.get();
        if (!stacks.isEmpty() && stacks.peek().playerId().equals(player.getUUID())) {
            stacks.pop();
        }
        if (stacks.isEmpty()) {
            ACTIVE_MINING_STACKS.remove();
        }
    }
