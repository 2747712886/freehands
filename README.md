...
- 右键方块使用：原版会依次处理主手和副手；只有副手对本次右键返回 `PASS` 时，解放槽中的锄、斧、锹或剪刀才作为最后回退执行右键方块操作，并消耗自身耐久。这样同一次右键不会因主、副手发包而重复执行饰品工具。
- 铁、钻石、下界合金饰品也可作为右键回退：支持铁锹铲平土径和熄灭营火、斧头去皮/刮锈/去蜡、锄头耕地，以及剪刀雕刻南瓜和修剪生长藤蔓（设为最大年龄并停止继续生长）。铁饰品自身固定按斧头、铁锹、锄头顺序尝试，潜行不会改变该顺序；成功时消耗饰品耐久并播放摆动动作。
- 右键优先级：放置方块、主手右键、副手右键、解放槽工具右键；解放槽工具只在副手 `PASS` 后按槽位顺序尝试，只有某槽返回 `PASS` 才继续下一槽，任何非 `PASS` 结果都会停止本次遍历。下一次右键会重新从首槽判断，可据此安排例如“先铲土径、再耕地”或“直接锄地”的顺序。
...
- 右键方块时，原版先处理主手，再处理副手；只有副手的完整方块交互返回 `PASS` 时才尝试解放槽工具，因此饰品工具每次物理右键只执行一次。
