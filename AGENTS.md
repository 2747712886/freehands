...
- Right-click block-use priority is fixed: block placement, main-hand use, off-hand use, then a free-hand tool only after the off-hand `useItemOn` returns `PASS`. Minecraft sends main-hand and off-hand interactions for an otherwise unhandled block click, so running the fallback from the main-hand call executes a free-hand tool twice. GameTests verify that main-hand dirt placement and an off-hand shovel both take priority over an equipped free-hand shovel.
...
- `freeHandToolRunsOnlyOnceAcrossHandPackets` simulates the main-hand and off-hand packets sent for one unhandled block click. A free-hand Iron Trinket must flatten dirt once, leaving a dirt path and consuming exactly one durability; it must not use its hoe action again from the off-hand packet.
...
- 主、副手均未处理右键方块操作时，解放槽中的锄、斧、锹、剪刀或能力饰品可作为最后回退执行受支持的右键方块行为并消耗自身耐久。能力饰品支持铁锹铲平/熄灭营火、斧头去皮/刮锈/去蜡、锄头耕地、剪刀雕刻南瓜/修剪生长藤蔓（设为最大年龄并停止继续生长），并在成功时摆动主手；能力饰品固定按斧头、铁锹、锄头顺序尝试，不受潜行影响。
...
- 右键实现：方块放置、主手右键和副手右键必须优先于解放槽工具。客户端会为未处理的方块右键依次发送主手和副手交互，因此不能从主手 `useItemOn` 回退到解放槽，否则同一次点击会重复执行工具。只有副手 `useItemOn` 返回 `PASS` 后才可按槽位顺序尝试解放槽物品；只有某槽返回 `PASS` 才继续下一槽，任何非 `PASS` 结果均停止本次遍历。下一次玩家右键重新从首槽开始。`UseOnContext` 读取 `getItemInHand(MAIN_HAND)`，虚拟主手必须覆盖该访问器。
