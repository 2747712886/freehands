...
- 未完成 / 下一步：① dev 客户端手工验收——Curios 两槽 UI、连锁右键手感、挥臂/音效与客户端预测（GameTest 覆盖不到）；② 客户端专用 devmods（JEI/IMBlocker/JustEnoughCharacters）尚未移植到 1.21.1（JEI/JEC 有 NeoForge 版可加进 `downloadDevelopmentMods`，IMBlocker 需确认）；③ 正文《常用命令》《验证清单》等仍混 1.20.1/Forge 口径，若要切 1.21.1 口径需用户点头；④ 可选：解放槽白名单补第三方工具标签、按需扩展 `#minecraft:enchantable/*` 标签。
